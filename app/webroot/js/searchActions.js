...
$(function () {
    ...
    Ric.searchActionsManager = function (config) {
        ...
        var $nomCompletElement = $('#SearchName');
        var $communeIdElement = $("#SearchCommuneId");
        var getCommunesUrl = config.getCommunesUrl || jsBase + "/../Communes/autocomplete";
        var getNomCompletUrl = config.getNomCompletUrl || $formElement.attr('action') + '/../getNomComplet';
        ...
        function setNomCompletAutoCompletion() {
            if($nomCompletElement.length>0){
            $nomCompletElement.autocomplete({
                source: getNomCompletUrl,
                minLength: 3,
                select: function (event, ui) {
                    $nomCompletElement.val(ui.item.label);
                ...
                return $("<li></li>")
                        .append("<a class=\"communes_autocomplete_label\">" + hightlightLabel + "</a>")
                        .appendTo(ul);
            };
        }
        }
        function setCommunesAutocompletion() {
            if($communeElement.length>0){
        ...
        function setCommunesNearSlider() {
            ...
            if (!$communeElement.val())
                $communesRadiusContainer.hide();
            else {
        ...
        function bindEvents() {
            ...
            $communeElement.blur(function () {
                if ($(this).val() !== currentCommune)
                    resetCommunes();
            });
            ...
            $tabContainer.on('shown.bs.tab', $('a[data-toggle="tab"]'), function (e) {
                localStorage[prefix + 'SelectedTab'] = $(e.target).attr('href');
            });
            $simpleSearchTab.on('click', (function () {
                $inputsFieldsToShowOnAdvancedSearch.hide();
            }));
            $advancedSearchTab.on('click', (function () {
        ...
        function setInitialSeachTab() {
            ...
            if (selectedTab)
                $tabContainer.find('a[href="' + selectedTab + '"]').tab('show');
