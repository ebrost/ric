		...
		if ($this->model->validates()){
			...
			if (count($results)>1){
				foreach($results as $result){
					$links[]=array(
						'url'=>Router::url('/',true).strtolower($plugin).'/'.$controller.'/'.$result[$model]['id'].'-'.Inflector::slug($result[$model]['nom_complet']),
				...
				$links[]=array(
				'url'=>Router::url('/',true).strtolower($plugin).'/'.$controller.'/'.$results[0][$model]['id'].'-'.Inflector::slug($results[0][$model]['nom_complet']),
